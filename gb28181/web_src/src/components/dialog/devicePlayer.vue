...
<div id="devicePlayer">
    ...
    <el-dialog title="视频播放" top="0" :close-on-click-modal="false" :visible.sync="showVideoDialog" @close="close()">
      <div style="width: 100%; height: 100%" v-loading="isLoging">
        ...
        <flvjs v-if="activePlayer == 'flvjs'" ref="flvjs" :visible.sync="showVideoDialog" :videoUrl="videoUrl"></flvjs>
      </div>
        <div id="shared" style="text-align: right; margin-top: 1rem;">
            <el-tabs v-model="tabActiveName" @tab-click="tabHandleClick" >
                <el-tab-pane label="实时视频" name="media" v-if="tabActiveName === 'media' || tabActiveName === 'control' || tabActiveName === 'codec'">
                      ...
                      <div style="width: 100%; text-align: left;color:#303133;">
                        ...
                        <el-button-group style="margin-left: 1rem;">
                          ...
                          <el-dropdown size="mini" title="播放倍速" style="margin-left: 1px;" @command="gbScale">
                            <el-button size="mini">
                              {{speedTxt}} 倍速 <i class="el-icon-arrow-down el-icon--right"></i>
                            </el-button>
                            <el-dropdown-menu  slot="dropdown">
                ...
                <!--遥控界面-->
                <el-tab-pane label="云台控制" name="control" v-if="tabActiveName === 'media' || tabActiveName === 'control' || tabActiveName === 'codec'">
                ...
                <el-tab-pane label="编码信息" name="codec" v-if="tabActiveName === 'media' || tabActiveName === 'control' || tabActiveName === 'codec'" v-loading="tracksLoading">
                    <!-- <p>
    ...
    <recordDownload v-if="tabActiveName === 'record'" ref="recordDownload"></recordDownload>
</div>
</template>
...
import flvjs from '../common/flvjs.vue'
    ...
    methods: {
        tabHandleClick: function(tab, event) {
            ...
            if (tab.name === "codec") {
              ...
              this.videoHistory.date = moment().format("yyyy-MM-DD");
              this.queryRecords()
            }
        },
        ...
        openDialog: function (tab, deviceId, channelId, param) {
            ...
            this.activePlayer = tab == 'record' ? 'flvjs' : 'jessibuca';
        ...
        stopPlayRecord: function (callback) {
          this.$refs[this.activePlayer].stop();
          this.videoUrl = '';
          this.$axios({
              method: 'get',
        ...
        gbPlay() {
          ...
          this.$axios({
            ...
            this.$refs[this.activePlayer].play();
          });
        },
        gbPause(){
          console.log('前端控制：暂停');
        ...
        gbScale(command){
          ...
          this.speedTxt = command;
          this.$axios({
            method: 'get',
            url: `/api/playback/speed/${this.streamId}/${command}`
          }).then((res) => {
            this.$refs[this.activePlayer].forward(parseFloat(command));
          });
        },
        gbSeek(val){
