package com.sescity.vbase.vmanager.gb28181.device;
...
import com.sescity.vbase.gb28181.bean.SyncStatus;
...
public class DeviceQuery {
	...
	public VbaseResult<SyncStatus> devicesSync(@PathVariable String deviceId){
		...
		if (status) {
			...
			return VbaseResult.success(channelSyncStatus);
		}
		deviceService.sync(device);

		VbaseResult<SyncStatus> vbaseResult = new VbaseResult<>();
		vbaseResult.setCode(0);
		vbaseResult.setMsg("开始同步");
		return vbaseResult;
	}
	...
	@Parameter(name = "deviceId", description = "设备国标编号", required = true)
	public VbaseResult<SyncStatus> getSyncStatus(@PathVariable String deviceId) {
		SyncStatus channelSyncStatus = deviceService.getChannelSyncStatus(deviceId);
		VbaseResult<SyncStatus> vbaseResult = new VbaseResult<>();
		if (channelSyncStatus == null) {
			vbaseResult.setCode(-1);
			vbaseResult.setMsg("同步尚未开始");
		}else {
			vbaseResult.setCode(ErrorCode.SUCCESS.getCode());
			vbaseResult.setMsg(ErrorCode.SUCCESS.getMsg());
			vbaseResult.setData(channelSyncStatus);
			if (channelSyncStatus.getErrorMsg() != null) {
				vbaseResult.setMsg(channelSyncStatus.getErrorMsg());
			}
		}
		return vbaseResult;
	}
	...
	@Parameter(name = "deviceId", description = "设备国标编号", required = true)
	public VbaseResult<Map<String, Integer>> getSubscribeInfo(@PathVariable String deviceId) {
		Set<String> allKeys = dynamicTask.getAllKeys();
		...
		VbaseResult<Map<String, Integer>> vbaseResult = new VbaseResult<>();
		vbaseResult.setCode(0);
		vbaseResult.setData(dialogStateMap);
		return vbaseResult;
	}
