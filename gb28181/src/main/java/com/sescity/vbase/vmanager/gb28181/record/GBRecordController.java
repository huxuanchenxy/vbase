package com.sescity.vbase.vmanager.gb28181.record;
...
public class GBRecordController {
	...
	public DeferredResult<VbaseResult<RecordInfo>> recordinfo(@PathVariable String deviceId, @PathVariable String channelId, String startTime, String endTime){

		if (logger.isDebugEnabled()) {
			logger.debug(String.format("录像信息查询 API调用，deviceId：%s ，startTime：%s， endTime：%s",deviceId, startTime, endTime));
		}
		DeferredResult<VbaseResult<RecordInfo>> result = new DeferredResult<>();
		...
		try {
			cmder.recordInfoQuery(device, channelId, startTime, endTime, sn, null, null, null, (eventResult -> {
				VbaseResult<RecordInfo> vbaseResult = new VbaseResult<>();
				vbaseResult.setCode(ErrorCode.ERROR100.getCode());
				vbaseResult.setMsg("查询录像失败, status: " +  eventResult.statusCode + ", message: " + eventResult.msg);
	...
	public DeferredResult<VbaseResult<StreamInfo>> download(@PathVariable String deviceId, @PathVariable String channelId,
		...
		DeferredResult<VbaseResult<StreamInfo>> result = new DeferredResult<>(30000L);
		resultHolder.put(key, uuid, result);
		RequestMessage msg = new RequestMessage();
		msg.setId(uuid);
		msg.setKey(key);
		...
		playService.download(deviceId, channelId, startTime, endTime, Integer.parseInt(downloadSpeed), null, playBackResult->{
			...
			vbaseResult.setCode(playBackResult.getCode());
			vbaseResult.setMsg(playBackResult.getMsg());
