package com.sescity.vbase.vmanager.streamPush;
...
import com.sescity.vbase.media.zlm.dto.StreamPushItem;
...
import com.sescity.vbase.service.impl.StreamPushUploadFileHandler;
import com.sescity.vbase.vmanager.bean.BatchGBStreamParam;
import com.sescity.vbase.vmanager.bean.ErrorCode;
import com.sescity.vbase.vmanager.bean.StreamPushExcelDto;
...
public class StreamPushController {
    ...
    @ResponseBody
    public DeferredResult<ResponseEntity<VbaseResult<Object>>> uploadChannelFile(@RequestParam(value = "file") MultipartFile file){

        // 最多处理文件一个小时
        DeferredResult<ResponseEntity<VbaseResult<Object>>> result = new DeferredResult<>(60*60*1000L);
        ...
        if (file.isEmpty()) {
            ...
            vbaseResult.setCode(-1);
            vbaseResult.setMsg("文件为空");
            result.setResult(ResponseEntity.status(HttpStatus.BAD_REQUEST).body(vbaseResult));
            return result;
        }
        if (file.getContentType() == null) {
            VbaseResult<Object> vbaseResult = new VbaseResult<>();
            vbaseResult.setCode(-1);
            vbaseResult.setMsg("无法识别文件类型");
            result.setResult(ResponseEntity.status(HttpStatus.BAD_REQUEST).body(vbaseResult));
            return result;
        }
        // 同时只处理一个文件
        if (resultHolder.exist(key, null)) {
            ...
            VbaseResult<Object> vbaseResult = new VbaseResult<>();
            vbaseResult.setCode(-1);
            vbaseResult.setMsg("已有导入任务正在执行");
            result.setResult(ResponseEntity.status(HttpStatus.TOO_MANY_REQUESTS).body(vbaseResult));
            return result;
        }
        ...
        result.onTimeout(()->{
            ...
            VbaseResult<Object> vbaseResult = new VbaseResult<>();
            vbaseResult.setCode(-1);
            vbaseResult.setMsg("导入超时，可能文件过大");
        ...
        try {
            ...
            ExcelReader excelReader = EasyExcel.read(inputStream, StreamPushExcelDto.class,
                    new StreamPushUploadFileHandler(streamPushService, mediaServerService.getDefaultMediaServer().getId(), (errorStreams, errorGBs)->{
                        ...
                        RequestMessage msg = new RequestMessage();
                        msg.setKey(key);
                        VbaseResult<Map<String, List<String>>> vbaseResult = new VbaseResult<>();
                        if (errorStreams.size() == 0 && errorGBs.size() == 0) {
                            vbaseResult.setCode(0);
                            vbaseResult.setMsg("成功");
                        }else {
                            vbaseResult.setCode(1);
                            vbaseResult.setMsg("导入成功。但是存在重复数据");
                            Map<String, List<String>> errorData = new HashMap<>();
                            errorData.put("gbId", errorGBs);
                            errorData.put("stream", errorStreams);
                            vbaseResult.setData(errorData);
                        }
                        msg.setData(vbaseResult);
                        resultHolder.invokeAllResult(msg);
                    })).build();
            ...
            VbaseResult<Object> vbaseResult = new VbaseResult<>();
            vbaseResult.setCode(-1);
            vbaseResult.setMsg("通道导入失败: " + e.getMessage() );
            msg.setData(vbaseResult);
            resultHolder.invokeAllResult(msg);
        }
