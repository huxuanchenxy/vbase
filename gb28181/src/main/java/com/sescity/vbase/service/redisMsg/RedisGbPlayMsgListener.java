package com.sescity.vbase.service.redisMsg;
...
import com.sescity.vbase.media.zlm.ZLMMediaListManager;
...
import com.sescity.vbase.service.bean.*;
...
public class RedisGbPlayMsgListener implements MessageListener {
    ...
    public static final String VBASE_PUSH_STREAM_KEY = "VBASE_PUSH_STREAM";
    ...
    public interface PlayMsgErrorCallback{
        void handler(VbaseResult vbaseResult);
    }
        ...
        if (!taskQueueHandlerRun) {
            ...
            taskExecutor.execute(() -> {
                while (!taskQueue.isEmpty()) {
                    ...
                    VbaseRedisMsg vbaseRedisMsg = JSON.toJavaObject(msgJSON, VbaseRedisMsg.class);
                    if (!userSetting.getServerId().equals(vbaseRedisMsg.getToId())) {
                        continue;
                    }
                    if (VbaseRedisMsg.isRequest(vbaseRedisMsg)) {
                        logger.info("[收到REDIS通知] 请求： {}", new String(msg.getBody()));

                        switch (vbaseRedisMsg.getCmd()){
                            case VbaseRedisMsgCmd.GET_SEND_ITEM:
                                RequestSendItemMsg content = JSON.toJavaObject((JSONObject) vbaseRedisMsg.getContent(), RequestSendItemMsg.class);
                                requestSendItemMsgHand(content, vbaseRedisMsg.getFromId(), vbaseRedisMsg.getSerial());
                                break;
                            case VbaseRedisMsgCmd.REQUEST_PUSH_STREAM:
                                RequestPushStreamMsg param = JSON.toJavaObject((JSONObject) vbaseRedisMsg.getContent(), RequestPushStreamMsg.class);;
                                requestPushStreamMsgHand(param, vbaseRedisMsg.getFromId(), vbaseRedisMsg.getSerial());
                                break;
                            default:
                                break;
                        }
                        ...
                        switch (vbaseRedisMsg.getCmd()){
                            case VbaseRedisMsgCmd.GET_SEND_ITEM:
                                ...
                                VbaseResult content  = JSON.toJavaObject((JSONObject) vbaseRedisMsg.getContent(), VbaseResult.class);

                                String key = vbaseRedisMsg.getSerial();
                                ...
                                break;
                            case VbaseRedisMsgCmd.REQUEST_PUSH_STREAM:
                                VbaseResult vbaseResult = JSON.toJavaObject((JSONObject) vbaseRedisMsg.getContent(), VbaseResult.class);
                                String serial = vbaseRedisMsg.getSerial();
                                switch (vbaseResult.getCode()) {
                                    case 0:
                                        JSONObject jsonObject = (JSONObject) vbaseResult.getData();
                                    ...
                                    case ERROR_CODE_TIMEOUT:
                                        ...
                                        if (errorCallback != null) {
                                            ...
                                            errorCallback.handler(vbaseResult);
                                        }
                                        break;
                                    default:
                                        break;
    ...
    private void responsePushStream(JSONObject content, String toId, String serial) {
        ...
        VbaseResult<JSONObject> result = new VbaseResult<>();
        result.setCode(0);
        result.setData(content);

        VbaseRedisMsg response = VbaseRedisMsg.getResponseInstance(userSetting.getServerId(), toId,
                VbaseRedisMsgCmd.REQUEST_PUSH_STREAM, serial, result);
        JSONObject jsonObject = (JSONObject)JSON.toJSON(response);
        RedisUtil.convertAndSend(VBASE_PUSH_STREAM_KEY, jsonObject);
    }
    ...
    private void requestSendItemMsgHand(RequestSendItemMsg content, String toId, String serial) {
        ...
        if (mediaServerItem == null) {
            ...
            VbaseResult<SendRtpItem> result = new VbaseResult<>();
            result.setCode(ERROR_CODE_MEDIA_SERVER_NOT_FOUND);
            result.setMsg("流媒体不存在");

            VbaseRedisMsg response = VbaseRedisMsg.getResponseInstance(userSetting.getServerId(), toId,
                    VbaseRedisMsgCmd.GET_SEND_ITEM, serial, result);
        ...
        if (streamReady) {
            ...
            dynamicTask.startDelay(taskKey, ()->{
                ...
                VbaseRedisMsg response = VbaseRedisMsg.getResponseInstance(
                        userSetting.getServerId(), toId, VbaseRedisMsgCmd.GET_SEND_ITEM, serial, result
                );
                JSONObject jsonObject = (JSONObject)JSON.toJSON(response);
                RedisUtil.convertAndSend(VBASE_PUSH_STREAM_KEY, jsonObject);
    ...
    private void responseSendItem(MediaServerItem mediaServerItem, RequestSendItemMsg content, String toId, String serial) {
        ...
        VbaseResult<ResponseSendItemMsg> result = new VbaseResult<>();
        result.setCode(0);
    ...
    public void sendMsg(String serverId, String mediaServerId, String app, String stream, String ip, int port, String ssrc,
        ...
        VbaseRedisMsg redisMsg = VbaseRedisMsg.getRequestInstance(userSetting.getServerId(), serverId, VbaseRedisMsgCmd.GET_SEND_ITEM,
        ...
        dynamicTask.startDelay(key, ()->{
            callbacks.remove(key);
            callbacksForError.remove(key);
            VbaseResult<Object> vbaseResult = new VbaseResult<>();
            vbaseResult.setCode(ERROR_CODE_TIMEOUT);
            vbaseResult.setMsg("timeout");
            errorCallback.handler(vbaseResult);
        }, userSetting.getPlatformPlayTimeout());
        RedisUtil.convertAndSend(VBASE_PUSH_STREAM_KEY, jsonObject);
    }
    ...
    public void sendMsgForStartSendRtpStream(String serverId, RequestPushStreamMsg param, PlayMsgCallbackForStartSendRtpStream callback) {
        String key = UUID.randomUUID().toString();
        VbaseRedisMsg redisMsg = VbaseRedisMsg.getRequestInstance(userSetting.getServerId(), serverId,
                VbaseRedisMsgCmd.REQUEST_PUSH_STREAM, key, param);
        ...
        callbacksForError.put(key, (vbaseResult)->{
            logger.info("[REDIS 请求其他平台推流] 失败: {}", vbaseResult.getMsg());
            callbacksForStartSendRtpStream.remove(key);
            callbacksForError.remove(key);
        });
        RedisUtil.convertAndSend(VBASE_PUSH_STREAM_KEY, jsonObject);
