...
public class MediaServerItem{
    ...
    @Schema(description = "hook使用的IP（zlm访问Vbase使用的IP）")
    ...
    @Schema(description = "隶属于哪个vbase，用于分组管理")
    private String vbaseId;
    ...
    public MediaServerItem(String vbaseId) {
      this.vbaseId = vbaseId;
    }

    public MediaServerItem(ZLMServerConfig zlmServerConfig, String sipIp, String vbaseId) {
        ...
        rtpPortRange = zlmServerConfig.getPortRange().replace("-",","); // 默认使用30000,30500作为级联时发送流的端口号
        sendRtpPortRange = zlmServerConfig.getPortRange().replace("-",","); // 默认使用30000,30500作为级联时发送流的端口号
    ...
    public String getVbaseId() {
      return vbaseId;
    }

    public void setVbaseId(String vbaseId) {
      this.vbaseId = vbaseId;
    }
}
