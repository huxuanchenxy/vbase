package com.sescity.vbase.storager.impl;
...
import com.sescity.vbase.media.zlm.dto.MediaItem;
import com.sescity.vbase.media.zlm.dto.MediaServerItem;
import com.sescity.vbase.media.zlm.dto.StreamAuthorityInfo;
import com.sescity.vbase.service.bean.GPSMsgInfo;
import com.sescity.vbase.service.bean.MessageForPushChannel;
import com.sescity.vbase.service.bean.ThirdPartyGB;
import com.sescity.vbase.storager.IRedisCacheStorage;
import com.sescity.vbase.storager.dao.DeviceChannelMapper;
import com.sescity.vbase.storager.dao.dto.PlatformRegisterInfo;
import com.sescity.vbase.utils.DateUtil;
import com.sescity.vbase.utils.SystemInfoUtils;
...
@Component
public class RedisCacheStorageImpl implements IRedisCacheStorage {

    private final Logger logger = LoggerFactory.getLogger(RedisCacheStorageImpl.class);
    ...
    public boolean startPlay(StreamInfo stream) {
        ...
        return RedisUtil.set(
                String.format("%S_%s_%s_%s_%s_%s", VideoManagerConstants.PLAYER_PREFIX, userSetting.getServerId(),
    ...
    public StreamInfo queryPlayByStreamId(String streamId) {
        List<Object> playLeys = RedisUtil.scan(String.format("%S_%s_*_%s_*", VideoManagerConstants.PLAYER_PREFIX,
                userSetting.getServerId(), streamId));
    ...
    public Map<String, StreamInfo> queryPlayByDeviceId(String deviceId) {
        ...
        List<Object> players = RedisUtil.scan(String.format("%S_%s_*_*_%s_*", VideoManagerConstants.PLAYER_PREFIX,
                userSetting.getServerId(), deviceId));
        if (players.size() == 0) {
            return streamInfos;
        }
    ...
    public boolean startPlayback(StreamInfo stream, String callId) {
        return RedisUtil.set(String.format("%S_%s_%s_%s_%s_%s_%s", VideoManagerConstants.PLAY_BLACK_PREFIX,
                userSetting.getServerId(), stream.getMediaServerId(), stream.getDeviceID(), stream.getChannelId(),
                stream.getStream(), callId), stream);
    }
    ...
    public void updatePlatformCatchInfo(ParentPlatformCatch parentPlatformCatch) {
        String key = VideoManagerConstants.PLATFORM_CATCH_PREFIX + userSetting.getServerId() + "_"
                + parentPlatformCatch.getId();
        RedisUtil.set(key, parentPlatformCatch);
    }

    @Override
    public ParentPlatformCatch queryPlatformCatchInfo(String platformGbId) {
        return (ParentPlatformCatch) RedisUtil
                .get(VideoManagerConstants.PLATFORM_CATCH_PREFIX + userSetting.getServerId() + "_" + platformGbId);
    }
    ...
    public PlatformRegisterInfo queryPlatformRegisterInfo(String callId) {
        return (PlatformRegisterInfo) RedisUtil
                .get(VideoManagerConstants.PLATFORM_REGISTER_INFO_PREFIX + userSetting.getServerId() + "_" + callId);
    }
    ...
    public void cleanPlatformRegisterInfos() {
        List regInfos = RedisUtil
                .scan(VideoManagerConstants.PLATFORM_REGISTER_INFO_PREFIX + userSetting.getServerId() + "_" + "*");
    ...
    public void clearCatchByDeviceId(String deviceId) {
        ...
        List<Object> playBackers = RedisUtil
                .scan(String.format("%S_%s_*_%s_*_*_*", VideoManagerConstants.PLAY_BLACK_PREFIX,
                        userSetting.getServerId(),
                        deviceId));
    ...
    @Override
    public void updateVbaseInfo(JSONObject jsonObject, int time) {
        String key = VideoManagerConstants.VBASE_SERVER_PREFIX + userSetting.getServerId();
        RedisUtil.set(key, jsonObject, time);
    }

    @Override
    public void sendStreamChangeMsg(String type, JSONObject jsonObject) {
        String key = VideoManagerConstants.VBASE_MSG_STREAM_CHANGE_PREFIX + type;
        logger.info("[redis 流变化事件] {}: {}", key, jsonObject.toString());
        RedisUtil.convertAndSend(key, jsonObject);
    }

    @Override
    public void addStream(MediaServerItem mediaServerItem, String type, String app, String streamId,
            MediaItem mediaItem) {
        // 查找是否使用了callID
        StreamAuthorityInfo streamAuthorityInfo = getStreamAuthorityInfo(app, streamId);
        String key = VideoManagerConstants.VBASE_SERVER_STREAM_PREFIX + userSetting.getServerId() + "_" + type + "_"
                + app + "_" + streamId + "_" + mediaServerItem.getId();
    ...
    public void removeStream(String mediaServerId, String type, String app, String streamId) {
        String key = VideoManagerConstants.VBASE_SERVER_STREAM_PREFIX + userSetting.getServerId() + "_" + type + "_"
                + app + "_" + streamId + "_" + mediaServerId;
        RedisUtil.del(key);
    }
    ...
    public ThirdPartyGB queryMemberNoGBId(String queryKey) {
        String key = VideoManagerConstants.VBASE_STREAM_GB_ID_PREFIX + queryKey;
    ...
    public void removeStream(String mediaServerId, String type) {
        String key = VideoManagerConstants.VBASE_SERVER_STREAM_PREFIX + userSetting.getServerId() + "_" + type + "_*_*_"
                + mediaServerId;
    ...
    public void updateDevice(Device device) {
        ...
        if (device.getNatHostAddress() != null) {
            String natIp = "";
            String natHostAddress = "";

            if(device.getNatHostAddress().indexOf(":")>=0)
            {
                natIp = device.getNatHostAddress().split(":")[0];
                natHostAddress = device.getNatHostAddress();
            }
            else
            {
                natIp = device.getNatHostAddress();
                natHostAddress = device.getNatHostAddress() + ":" + device.getPort();
            }

            device.setHostAddress(natHostAddress);
            device.setIp(natIp);
            // device.setHostAddress(device.getNatHostAddress());
            // device.setIp("101.95.55.9");
        }
    ...
    public void updateGpsMsgInfo(GPSMsgInfo gpsMsgInfo) {
        String key = VideoManagerConstants.VBASE_STREAM_GPS_MSG_PREFIX + userSetting.getServerId() + "_"
                + gpsMsgInfo.getId();
    ...
    public GPSMsgInfo getGpsMsgInfo(String gbId) {
        String key = VideoManagerConstants.VBASE_STREAM_GPS_MSG_PREFIX + userSetting.getServerId() + "_" + gbId;
        return (GPSMsgInfo) RedisUtil.get(key);
    }

    @Override
    public List<GPSMsgInfo> getAllGpsMsgInfo() {
        String scanKey = VideoManagerConstants.VBASE_STREAM_GPS_MSG_PREFIX + userSetting.getServerId() + "_*";
    ...
    public void updateStreamAuthorityInfo(String app, String stream, StreamAuthorityInfo streamAuthorityInfo) {
        String key = VideoManagerConstants.MEDIA_STREAM_AUTHORITY + userSetting.getServerId() + "_" + app + "_"
                + stream;
    ...
    public MediaItem getStreamInfo(String app, String streamId, String mediaServerId) {
        String scanKey = VideoManagerConstants.VBASE_SERVER_STREAM_PREFIX + userSetting.getServerId() + "_*_" + app
                + "_" + streamId + "_" + mediaServerId;
    ...
    public int getPushStreamCount(String id) {
        String key = VideoManagerConstants.VBASE_SERVER_STREAM_PREFIX + userSetting.getServerId() + "_PUSH_*_*_" + id;
        return RedisUtil.scan(key).size();
    }

    @Override
    public int getProxyStreamCount(String id) {
        String key = VideoManagerConstants.VBASE_SERVER_STREAM_PREFIX + userSetting.getServerId() + "_PULL_*_*_" + id;
        return RedisUtil.scan(key).size();
    }
    ...
    public int getGbReceiveCount(String id) {
        ...
        String playBackKey = VideoManagerConstants.PLAY_BLACK_PREFIX + "_" + userSetting.getServerId() + "_" + id
                + "_*";
