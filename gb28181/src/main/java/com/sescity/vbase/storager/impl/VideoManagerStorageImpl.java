...
import com.sescity.vbase.gb28181.event.subscribe.catalog.CatalogEvent;
import com.sescity.vbase.media.zlm.dto.StreamProxyItem;
import com.sescity.vbase.service.IGbStreamService;
...
import com.sescity.vbase.storager.dao.*;
import com.sescity.vbase.storager.dao.dto.ChannelSourceInfo;
...
public class VideoManagerStorageImpl implements IVideoManagerStorage {
	...
	public Device queryVideoDevice(String deviceId) {
		...
		if (device != null && !StringUtils.equals(device.getNatHostAddress(), null)) {
	...
	@Override
	public PageInfo queryChannelsByDeviceId(String deviceId, String query, Boolean hasSubChannel, Boolean online,
			Boolean catalogUnderDevice, int page, int count) {
		// 获取到所有正在播放的流
	...
	@Override
	public List<DeviceChannel> queryChannelsByDeviceIdWithStartAndLimit(String deviceId, String query,
			Boolean hasSubChannel, Boolean online, int start, int limit) {
		return deviceChannelMapper.queryChannelsByDeviceIdWithStartAndLimit(deviceId, null, query, hasSubChannel,
				online, start, limit);
	}
	...
	@Override
	public PageInfo<DeviceChannel> querySubChannels(String deviceId, String parentChannelId, String query,
			Boolean hasSubChannel, Boolean online, int page, int count) {
		PageHelper.startPage(page, count);
		List<DeviceChannel> all = deviceChannelMapper.queryChannels(deviceId, parentChannelId, query, hasSubChannel,
				online);
		return new PageInfo<>(all);
	}
	 ...
	 */
	@Override
	public synchronized List<MobilePosition> queryMobilePositions(String deviceId, String channelId, String startTime,
			String endTime) {
	...
	public boolean updateParentPlatform(ParentPlatform parentPlatform) {
		...
		ParentPlatformCatch parentPlatformCatch = redisCatchStorage
				.queryPlatformCatchInfo(parentPlatform.getServerGBId()); // .getDeviceGBId());
	...
	public PageInfo<ChannelReduce> queryAllChannelList(int page, int count, String query, Boolean online,
		...
		PageHelper.startPage(page, count);
		List<ChannelReduce> all = deviceChannelMapper.queryChannelListInAll(query, online, channelType, platformId,
				catalogId);
		return new PageInfo<>(all);
	}
	...
	public List<PlatformCatalog> queryChannelInParentPlatformAndCatalog(String platformId, String catalogId) {
		List<PlatformCatalog> catalogs = platformChannelMapper.queryChannelInParentPlatformAndCatalog(platformId,
				catalogId);
		return catalogs;
	}
	...
	public List<PlatformCatalog> queryStreamInParentPlatformAndCatalog(String platformId, String catalogId) {
		List<PlatformCatalog> catalogs = platformGbStreamMapper
				.queryChannelInParentPlatformAndCatalogForCatalog(platformId, catalogId);
		return catalogs;
	}
	...
	public Device queryVideoDeviceByChannelId(String channelId) {
		...
		if (channelList.size() == 1) {
			...
			if (result != null && !StringUtils.equals(result.getNatHostAddress(), null)) {
				result.setHostAddress(result.getNatHostAddress());
			}
		}
		return result;
	}
	...
	public int delCatalog(String id) {
		...
		if (platformCatalog.getChildrenCount() > 0) {
			List<PlatformCatalog> platformCatalogList = catalogMapper.selectByParentId(platformCatalog.getPlatformId(),
					platformCatalog.getId());
	...
	private int delCatalogExecute(String id, String platformId) {
		...
		List<PlatformCatalog> platformCatalogs = platformChannelMapper
				.queryChannelInParentPlatformAndCatalog(platformId, id);
	...
	public int delRelation(PlatformCatalog platformCatalog) {
		if (platformCatalog.getType() == 1) {
			...
			List<GbStream> gbStreams = platformGbStreamMapper.queryChannelInParentPlatformAndCatalog(
					platformCatalog.getPlatformId(), platformCatalog.getParentId());
			for (GbStream gbStream : gbStreams) {
				if (gbStream.getGbId().equals(platformCatalog.getId())) {
					DeviceChannel deviceChannel = new DeviceChannel();
					deviceChannel.setChannelId(gbStream.getGbId());
					eventPublisher.catalogEventPublish(platformCatalog.getPlatformId(), deviceChannel,
							CatalogEvent.DEL);
	...
	private List<DeviceChannel> getDeviceChannelListByChannelReduceList(List<ChannelReduce> channelReduces,
			String catalogId) {
		List<DeviceChannel> deviceChannelList = new ArrayList<>();
	...
	private DeviceChannel getDeviceChannelByCatalog(PlatformCatalog catalog) {
		...
		deviceChannel.setManufacture("sescity");
		...
		deviceChannel.setOwner("sescity");
	...
	public void cleanContentForPlatform(String serverGBId) {
		// List<PlatformCatalog> catalogList =
		// catalogMapper.selectByPlatForm(serverGBId);
		...
		// }
		// eventPublisher.catalogEventPublish(serverGBId, deviceChannels,
		// CatalogEvent.DEL);
		// }
		// }
		catalogMapper.delByPlatformId(serverGBId);
