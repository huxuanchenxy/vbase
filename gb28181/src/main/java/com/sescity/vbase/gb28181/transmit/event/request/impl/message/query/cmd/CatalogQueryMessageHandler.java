package com.sescity.vbase.gb28181.transmit.event.request.impl.message.query.cmd;
...
import com.sescity.vbase.gb28181.transmit.event.request.impl.message.query.QueryMessageHandler;
...
public class CatalogQueryMessageHandler extends SIPRequestProcessorParent implements InitializingBean, IMessageHandler {
    ...
    @Autowired
    private SIPCommanderForPlatform cmderForPlatform;
...
//            allChannels.add(deviceChannel);
        ...
        try {
            if (allChannels.size() > 0) {
                cmderForPlatform.catalogQuery(allChannels, parentPlatform, sn, fromHeader.getTag());
            }else {
                // 回复无通道
                cmderForPlatform.catalogQuery(null, parentPlatform, sn, fromHeader.getTag(), 0);
            }
        } catch (SipException | InvalidArgumentException | ParseException e) {
    ...
    private DeviceChannel getChannelForPlatform(ParentPlatform platform) {
        ...
        deviceChannel.setAddress("sescity");
