...
import com.sescity.vbase.gb28181.session.RecordDataCache;
...
public class RecordInfoResponseMessageHandler extends SIPRequestProcessorParent implements InitializingBean, IMessageHandler {
    ...
    public void handForDevice(RequestEvent evt, Device device, Element rootElement) {
        ...
        taskQueue.offer(new HandlerCacheData(evt, device, rootElement));
                ...
                while (!taskQueue.isEmpty()) {
                    try {
                        ...
                        if (recordListElement == null || sumNum == 0) {
                            ...
                            recordDataCache.put(take.getDevice().getDeviceId(), sn, sumNum, new ArrayList<>());
                            ...
                            if (recordListIterator != null) {
                                ...
                                eventPublisher.recordEndEventPush(recordInfo);
                                int count = recordDataCache.put(take.getDevice().getDeviceId(), sn, sumNum, recordList);
                                logger.info("[国标录像]， {}->{}: {}/{}", take.getDevice().getDeviceId(), sn, count, sumNum);
                            }

                            if (recordDataCache.isComplete(take.getDevice().getDeviceId(), sn)){
                                releaseRequest(take.getDevice().getDeviceId(), sn);
                            }
                        }
    ...
    public void releaseRequest(String deviceId, String sn){
        ...
        RecordInfo recordInfo = recordDataCache.getRecordInfo(deviceId, sn);
        ...
        recordDataCache.remove(deviceId, sn);
    }
}
