package com.sescity.vbase.gb28181.event.subscribe.catalog;
 ...
 */
@Component
public class CatalogEventListener implements ApplicationListener<CatalogEvent> {

    private final static Logger logger = LoggerFactory.getLogger(CatalogEventListener.class);
            ...
            case CatalogEvent.DEL:
                ...
                if (parentPlatform != null || subscribe != null) {
                    ...
                    if (deviceChannelList.size() > 0) {
                        ...
                        try {
                            sipCommanderForPlatform.sendNotifyForCatalogOther(event.getType(), parentPlatform, deviceChannelList, subscribe, null);
                    ...
                    for (String gbId : parentPlatformMap.keySet()) {
                        ...
                        if (parentPlatforms != null && parentPlatforms.size() > 0) {
                            for (ParentPlatform platform : parentPlatforms) {
                                ...
                                try {
                                    sipCommanderForPlatform.sendNotifyForCatalogOther(event.getType(), platform, deviceChannelList, subscribeInfo, null);
                                } catch (InvalidArgumentException | ParseException | NoSuchFieldException | SipException |
                                         IllegalAccessException e) {
                                    logger.error("[命令发送失败] 国标级联 Catalog通知: {}", e.getMessage());
                                }
            ...
            case CatalogEvent.UPDATE:
                if (parentPlatform != null || subscribe != null) {
                    ...
                    if (deviceChannelList.size() > 0) {
                        logger.info("[Catalog事件: {}]平台：{}，影响通道{}个", event.getType(), event.getPlatformId(), deviceChannelList.size());
                        try {
                            sipCommanderForPlatform.sendNotifyForCatalogAddOrUpdate(event.getType(), parentPlatform, deviceChannelList, subscribe, null);
                        } catch (InvalidArgumentException | ParseException | NoSuchFieldException | SipException |
                                 IllegalAccessException e) {
                            logger.error("[命令发送失败] 国标级联 Catalog通知: {}", e.getMessage());
                        }
                    ...
                    for (String gbId : parentPlatformMap.keySet()) {
                        ...
                        if (parentPlatforms != null && parentPlatforms.size() > 0) {
                            for (ParentPlatform platform : parentPlatforms) {
                                ...
                                try {
                                    sipCommanderForPlatform.sendNotifyForCatalogAddOrUpdate(event.getType(), platform, deviceChannelList, subscribeInfo, null);
