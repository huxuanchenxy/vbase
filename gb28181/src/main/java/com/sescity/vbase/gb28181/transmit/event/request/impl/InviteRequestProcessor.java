package com.sescity.vbase.gb28181.transmit.event.request.impl;
...
import com.sescity.vbase.gb28181.transmit.SIPProcessorObserver;
import com.sescity.vbase.gb28181.transmit.SIPSender;
import com.sescity.vbase.gb28181.transmit.cmd.impl.SIPCommanderForPlatform;
import com.sescity.vbase.gb28181.transmit.event.request.ISIPRequestProcessor;
import com.sescity.vbase.gb28181.transmit.event.request.SIPRequestProcessorParent;
import com.sescity.vbase.gb28181.utils.SipUtils;
...
import com.sescity.vbase.media.zlm.dto.*;
import com.sescity.vbase.service.IMediaServerService;
import com.sescity.vbase.service.IPlayService;
import com.sescity.vbase.service.IStreamProxyService;
import com.sescity.vbase.service.IStreamPushService;
import com.sescity.vbase.service.bean.MessageForPushChannel;
import com.sescity.vbase.service.bean.SSRCInfo;
import com.sescity.vbase.service.redisMsg.RedisGbPlayMsgListener;
import com.sescity.vbase.service.redisMsg.RedisPushStreamResponseListener;
...
//                    }
                        ...
                        if (playTransaction != null) {
                            ...
                            if (!streamReady) {
                                ...
                                if (hasRtpServer) {
                                    ...
                                    HookSubscribeForStreamChange hookSubscribe = HookSubscribeFactory.on_stream_changed("rtp", playTransaction.getStream(), true, "rtmp", mediaServerItem.getId());
    ...
    private void pushStream(RequestEvent evt, SIPRequest request, GbStream gbStream, StreamPushItem streamPushItem, ParentPlatform platform,
        ...
        if (streamPushItem.isSelf()) {
            ...
            // 其他平台内容
            otherVbasePushStream(evt, request, gbStream, streamPushItem, platform, callIdHeader, mediaServerItem, port, tcpActive,
                    mediaTransmissionTCP, channelId, addressStr, ssrc, requesterId);
        }
    }
    /**
    ...
    private void notifyStreamOnline(RequestEvent evt, SIPRequest request, GbStream gbStream, StreamPushItem streamPushItem, ParentPlatform platform,
        ...
        if ("proxy".equals(gbStream.getStreamType())) {
            ...
            // 监听流上线
            HookSubscribeForStreamChange hookSubscribe = HookSubscribeFactory.on_stream_changed(gbStream.getApp(), gbStream.getStream(), true, "rtmp", mediaServerItem.getId());
     ...
     * 来自其他vbase的推流
     */
    private void otherVbasePushStream(RequestEvent evt, SIPRequest request, GbStream gbStream, StreamPushItem streamPushItem, ParentPlatform platform,
                                      CallIdHeader callIdHeader, MediaServerItem mediaServerItem,
                                      int port, Boolean tcpActive, boolean mediaTransmissionTCP,
                                      String channelId, String addressStr, String ssrc, String requesterId) {
        ...
        redisGbPlayMsgListener.sendMsg(streamPushItem.getServerId(), streamPushItem.getMediaServerId(),
                ...
                channelId, mediaTransmissionTCP, null, responseSendItemMsg -> {
                    ...
                    // 错误
                    if (vbaseResult.getCode() == RedisGbPlayMsgListener.ERROR_CODE_OFFLINE) {
                        // 离线
                        // 查询是否在本机上线了
