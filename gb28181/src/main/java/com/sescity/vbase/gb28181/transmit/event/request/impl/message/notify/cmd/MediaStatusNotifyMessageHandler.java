package com.sescity.vbase.gb28181.transmit.event.request.impl.message.notify.cmd;
...
import com.sescity.vbase.gb28181.bean.SsrcTransaction;
...
import com.sescity.vbase.gb28181.transmit.event.request.impl.message.notify.NotifyMessageHandler;
...
import static com.sescity.vbase.gb28181.utils.XmlUtil.getText;
        ...
        if ("121".equals(NotifyType)){
            ...
            if (ssrcTransaction != null) { // 兼容海康 媒体通知 消息from字段不是设备ID的问题
                ...
                if (sendRtpItem != null) {
                    ...
                    try {
                        sipCommanderForPlatform.sendMediaStatusNotify(parentPlatform, sendRtpItem);
                    } catch (SipException | InvalidArgumentException | ParseException e) {
                        logger.error("[命令发送失败] 国标级联 录像播放完毕: {}", e.getMessage());
                    }
                }
            } else {
                logger.info("[录像流]推送完毕，未找到SsrcTransaction {}", callIdHeader.getCallId());
            }
        }
    }
